...
import { getLocationEntities } from './location.reducer';
...
export class Location extends React.Component<ILocationProps, ILocationState> {
  ...
  render() {
    const { locationList, match, totalItems, restaurantList, account } = this.props;
      let isAdmin = false;
      let isUser = false;
      let finalListOfLocations : ReadonlyArray<ILocation> = [];
      ...
      if (isAdmin) {
          finalListOfLocations = locationList;
      } else if (isUser) {
          restaurantList.filter(restaurant => account.id === restaurant.userId).map( restaurant => {
              finalListOfLocations = finalListOfLocations.concat(locationList.filter( location => restaurant.idLocationId === location.id));
          })
      }
      ...
      <div>
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {finalListOfLocations.map((location, i) => (
                <tr key={`entity-${i}`}>
                  <td>
...
const mapStateToProps = ({ location, restaurant, authentication }: IRootState) => ({
...
const mapDispatchToProps = {
  getEntities: getLocationEntities,
