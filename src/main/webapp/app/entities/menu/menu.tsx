...
import { getMenuEntities } from './menu.reducer';
...
export class Menu extends React.Component<IMenuProps, IMenuState> {
  ...
  render() {
    const { menuList, match, totalItems, restaurantList, account } = this.props;
      let isAdmin = false;
      let isUser = false;
      let finalListOfMenus : ReadonlyArray<IMenu> = [];
      ...
      if (isAdmin) {
          finalListOfMenus = menuList;
      } else if (isUser) {
          restaurantList.filter(restaurant => account.id === restaurant.userId).map( restaurant => {
              finalListOfMenus = finalListOfMenus.concat(menuList.filter( menu => restaurant.id === menu.restaurantId));
          })
      }
...
const mapStateToProps = ({ menu, restaurant, authentication }: IRootState) => ({
